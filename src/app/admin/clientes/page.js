...
export default async function AdminClientesPage() {
  ...
  const storesWithStats = stores.map(store => {
    ...
    // Churn Risk calculation (simples: sem pedidos nos últimos 3 dias e tem token)
    const recentOrders = storeOrders.filter(o => {
      const diff = new Date() - new Date(o.purchase_date);
      return diff < 3 * 24 * 60 * 60 * 1000;
    });
    
    const churnRisk = store.access_token && recentOrders.length === 0;
    ...
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '24px', animation: 'fadeIn 0.5s ease-out' }}>
      <header style={{ marginBottom: '32px' }}>
        <h1 style={{ margin: '0 0 8px 0', fontSize: '2rem', display: 'flex', alignItems: 'center', gap: '12px' }}>
          <Users size={28} color="#8b5cf6" /> Gestão de Lojas e Clientes
        </h1>
        <p style={{ margin: 0, color: 'var(--text-secondary)' }}>Gerencie os planos, acompanhe a saúde e preste suporte.</p>
      </header>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
            <thead style={{ backgroundColor: 'var(--background)', borderBottom: '1px solid var(--border)' }}>
              <tr>
                ...
                <th style={{ padding: '16px 24px', fontWeight: '500', color: 'var(--text-secondary)' }}>Status (Health)</th>
                <th style={{ padding: '16px 24px', fontWeight: '500', color: 'var(--text-secondary)' }}>Métricas (Pós-App)</th>
                <th style={{ padding: '16px 24px', fontWeight: '500', color: 'var(--text-secondary)' }}>Ações / Plano</th>
              </tr>
            </thead>
            <tbody>
              {storesWithStats.map((store) => (
                <tr key={store.id} style={{ borderBottom: '1px solid var(--border)', transition: 'background-color 0.2s', backgroundColor: store.churnRisk ? 'rgba(239, 68, 68, 0.05)' : 'transparent' }}>
                  ...
                  <td style={{ padding: '16px 24px' }}>
                    {store.isConnected ? (
                      <span className="badge success">API Conectada</span>
                    ) : (
                      <span className="badge danger">Falta Instalar</span>
                    )}
                    {store.churnRisk && (
                      <div style={{ marginTop: '8px' }}>
                        <span className="badge warning">Risco de Churn (Inativo)</span>
                      </div>
                    )}
                  ...
                  <td style={{ padding: '16px 24px' }}>
                    <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>Pedidos Monitorados: <strong style={{color:'var(--text-primary)'}}>{store.totalOrders}</strong></div>
                  ...
                  <td style={{ padding: '16px 24px' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                      <span className={`badge ${store.current_plan === 'max' ? 'success' : store.current_plan === 'pro' ? 'warning' : 'info'}`} style={{ textTransform: 'capitalize', alignSelf: 'flex-start' }}>
